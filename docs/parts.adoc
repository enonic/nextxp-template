...
+
After the app has deployed, head over to Content Studio. This time, lets edit the `Persons` page. Drag'n drop the new part into the region (`right-click -> Insert`, or drag-n-drop from the `Insert panel`` on the right).
+
