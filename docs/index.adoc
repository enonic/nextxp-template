...
+
TIP: Visiting these link will give you a 404 page because the GraphQL APIs communicate via HTTP `POST` requests rather than `GET`.
...
+
KEY:VALUE:: CONTENT_API_DRAFT:<URL to the drafts API>
KEY:VALUE:: CONTENT_API_MASTER:<URL to the live API> 
...
+
NextJS server URL:: <absolute url to your vercel app>
